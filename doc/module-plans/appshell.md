...
当前最小链路支持 Agent 列表、创建表单、选择 Agent、编辑名称、按 Settings 白名单选择模型 provider、编辑模型 name、
编辑自定义 Agent 的私有 system prompt、选择 knowledge/skills/tools 和保存。Agent 编辑页通过
`AppResourceClient` 加载共享资源列表，保存时把选择结果写入 Agent manifest，并继续保留权限配置。
默认 `coding-agent` 表示内置 Pi coding agent，只允许编辑模型配置。

### Settings UI

- 展示支持的模型 provider。
- 按 API Key 授权和 OAuth/订阅授权分组。
...
`AppProviderAuthClient` 读写 Pi `auth.json` 的 API Key 凭据；Agent 编辑页通过 `AppSettingsClient`
加载 provider 白名单，并只允许选择白名单内的 provider。
...
- Agent 编辑页当前支持名称、Settings 白名单内的模型 provider、模型 name、system prompt 和 knowledge/skills/tools 选择；
  默认 `coding-agent` 只展示模型 provider/name。
...
- Settings 页面按 API Key 和 OAuth/订阅分组展示内置 Pi provider；当前实现 API Key 连接和断开，
  并通过 `AppProviderAuthClient` 写入 Pi `auth.json`，同时用 `AppSettingsClient` 同步 Agent 可使用
  的模型 provider 白名单。
...
凭据；OAuth/订阅授权后续再接入。`auth.json` 不要提交到仓库，后续可迁移到 Keychain 或更完整的
凭据管理。
...
- 用户可以在 Settings 中通过 API Key 连接 provider，并在 Agent 编辑页从同步后的白名单中选择。
