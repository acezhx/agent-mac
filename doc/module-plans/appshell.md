...
- 初始竖切不做 Agent 选择、资源选择、复杂权限 UI 或完整审批流程；当前已补上 Agent/Resource
  管理和工具审批 UI，仍未接入 Agent 编辑页资源选择和可配置 Agent session。
...
初始化 `FileStore`，创建 Application Support 下的基础目录和 `settings.yaml`。启动初始化失败
时，主窗口顶部展示可复制的错误提示；Runtime 缺少 Node、Runtime Host、Pi runtime、进程退出、
响应超时以及常见 Pi 模型/认证问题会通过 `AppSessionClientError` 映射成带修复方向的提示。
