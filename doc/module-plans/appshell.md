...
- UI 可创建默认 Pi coding agent session 并自动启动 Runtime Host session、发送消息、展示 snapshot
  中的用户/assistant/diagnostic 消息变化。
- UI 展示 idle/running/failed/aborted 以及 create/start/send/cancel turn 的进行中状态；`Start` 和
  `Reset` 不作为用户可见按钮暴露，composer 中的 Stop 表示取消当前轮输出，不表示 abort 整个 session。
- 第一阶段会话页只维护一个当前可见 session；创建后当前 session 的 Agent 和 workspace 锁定，
  `New Session` 回到启动 composer。启动 App 时不自动加载历史 session，项目列表在未选择文件夹前为空。
- 初始竖切不做 Agent 选择、资源选择、复杂权限 UI 或完整审批流程；当前已补上 Agent/Resource
  管理、Agent 编辑页资源选择、工具审批 UI，以及主会话创建时的 Agent 选择。Runtime/Session 已支持
  默认 Pi coding agent 与 resolved 自定义 Agent 两种形态。
...
- 根视图。
- 面向 Agent 使用场景的会话工作台。
- 左侧 Projects -> Sessions 层级侧栏；第一版只展示当前选择的项目，未选择文件夹时为空。
- Agent Library 窗口入口。
...
只允许编辑 skills；名称、system prompt、knowledge、tools、权限和模型都按 Pi coding agent 默认值处理。
...
- 选择 Agent。
- 选择 workspace。
- 新建 session 时展示居中的启动 composer；没有选择文件夹时标题显示“我们该做什么?”，
  文件夹选择按钮显示“进入项目工作”，项目列表为空。
- 选择文件夹后以文件夹名称作为项目名称，标题显示“我们应该在<项目名>中做些什么?”。
- 启动 composer 底部先展示文件夹选择，再展示 Agent 选择。
- 未选择文件夹时也可以直接开始对话；创建 session 时会自动使用
  `~/Documents/Codex/yyyy-MM-dd/new-chat` 作为默认项目目录，并在创建后显示为当前项目。
- 用户在启动 composer 输入首条消息并发送后，AppShell 创建 session、自动启动 Runtime Host session，
  再发送这条消息。
- 创建 session 后自动启动 Runtime Host session，并锁定当前 session 的 Agent 和 workspace。
- 发送按钮在当前轮运行时切换为 Stop；Stop 取消当前轮输出，保留 session、workspace、Agent 和已有消息。
- New Session 入口只放在左侧侧栏顶部，点击后回到启动 composer；历史 session 浏览和历史信息存储策略后续单独设计。
- 消息列表。
- 消息输入框。
- 发送按钮。
- 流式 assistant 输出。
...
- [x] 创建会话 Feature。
- [x] 实现会话页面。
- [x] 实现 Projects -> Sessions 侧栏。
- [x] 实现创建 session 前选择 Agent。
- [x] 实现新建 session 启动 composer。
- [x] 实现消息输入。
- [x] 实现当前轮输出取消。
- [x] 实现流式输出展示。
- [x] 实现基础错误提示。
...
- 当前主窗口通过 toolbar 和侧栏底部入口打开独立的 Agent Library、Resource Library 和 Settings 窗口，
  主窗口自身保持为会话工作台。
...
- Agent 编辑页当前支持名称、Settings 白名单内的模型 provider、RuntimeHost/Pi 模型清单中的模型
  name、system prompt 和 knowledge/skills/tools 选择；默认 `coding-agent` 只展示 skills 选择。
...
- 当前不做资源版本管理和语法高亮；Agent 编辑页资源选择已接入，主会话创建时已支持选择默认 Pi
  coding agent 或自定义 Agent。
...
已有的同 ID Agent。默认 `coding-agent` 的编辑页只展示 skills 选择；名称、system prompt、
knowledge、tools、权限和模型按 Pi coding agent 当前运行模式处理，不作为可编辑字段暴露。当前
Runtime Host 固定 Pi session 启用 Pi 内建 `read`、`bash`、`edit`、`write` 工具，并通过现有审批流
控制执行；同时关闭外部 Pi extensions、prompt templates、themes、项目 context files 和 Pi 默认 skills
自动发现，只加载默认 `coding-agent` 显式选择的 skills。启动初始化失败时，
...
- Feature 通过 `AppSessionClient` 这个 TCA dependency 调用 session 能力。
- 用户首次启动时看不到历史 session；选择文件夹后项目列表出现以文件夹名命名的项目。
- 用户可以选择 workspace 和 Agent，并通过启动 composer 的首条消息创建 session。
- 创建 session 后会自动启动 Runtime Host session。
- 用户可以发送一条消息。
- assistant 回复可以通过 `ChatSessionSnapshot` 的消息变化显示。
- running、failed、aborted 和基础操作进行中状态在 UI 中可见。
- Stop 有最小可用路径；Reset 仅保留为底层调试/恢复能力，不在 UI 暴露。
