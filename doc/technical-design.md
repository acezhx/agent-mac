...
- 应用启动和根视图组合。
- 窗口布局和导航。
- 在 Agent 管理、资源管理、会话视图之间切换。
- 持有应用级 UI 状态，例如当前选择的 Agent、workspace、当前页面。
- 将 UI 操作连接到底层应用服务。
...
请求，`Session` 能收到该事件，并通过默认拒绝策略返回结果。这样后续增加真正的审批 UI 时，
不需要重做 session 和 runtime event 通道。
...
- 遇到需要审批的工具调用，返回 denied/unsupported。
...
第一阶段使用默认拒绝实现。后续新增 `ApprovalService` 时替换这个实现即可。
